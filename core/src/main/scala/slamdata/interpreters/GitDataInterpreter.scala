...
class GitDataInterpreter [F[_] : Sync](impl: github4s.algebras.GitData[F])
  ...
  def updateReference(owner: String,
                      repo: String,
                      ref: String,
                      sha: String,
                      force: Boolean,
                      headers: Map[String, String]): F[GHResponse[Ref]] = {
  ...
  def createCommit(owner: String,
                   repo: String,
                   message: String,
                   tree: String,
                   parents: List[String],
                   author: Option[RefAuthor],
                   headers: Map[String, String]): F[GHResponse[RefCommit]] = {
    impl.createCommit(owner, repo, message, tree, parents, author, headers)
  }

  def createBlob(owner: String,
                 repo: String,
                 content: String,
                 encoding: Option[String],
                 headers: Map[String, String]): F[GHResponse[RefInfo]] = {
    impl.createBlob(owner, repo, content, encoding, headers)
  }

  def getTree(owner: String,
              repo: String,
              sha: String,
              recursive: Boolean,
              headers: Map[String, String]): F[GHResponse[TreeResult]] = {
    impl.getTree(owner, repo, sha, recursive, headers)
  }

  def createTree(owner: String,
                 repo: String,
  ...
  def createTag(owner: String,
                ...
                objectType: String,
                author: Option[RefAuthor],
                headers: Map[String, String]): F[GHResponse[Tag]] = {
